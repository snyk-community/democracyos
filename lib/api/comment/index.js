...
app.get('/comment/all', requireTopicId, function (req, res) {
  ...
  var author = req.query.author === ''
             ? req.query.author
             : req.query.excludeAuthor
             ? {'$ne': req.query.excludeAuthor}
             : null
  ...
  api.comment.all({
    ...
    if (!req.query.count) {
      ...
      var keys = [
        'id alias count text createdAt editedAt context reference',
        'author.id author.fullName author.displayName author.avatar author.staff author.roles author.status',
        'flags upvotes downvotes votes replies.length category'
      ].join(' ')
