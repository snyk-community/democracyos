...
app.get('/comment/all.csv', restrict, getForum, privileges('canChangeTopics'), function (req, res) {
  ...
  api.comment.all({forum: req.query.forumId}, function (err, comments) {
    if (err) {
...
function getForum (req, res, next) {
  api.forum.findById(req.query.forumId, function (err, forum) {
    if (err || !forum) {
      log('Error found: %j', err)
      return res.redirect('/500')
    }
