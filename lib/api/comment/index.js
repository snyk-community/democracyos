...
app.get('/comment/:forum/all.csv', restrict, getForum, privileges('canChangeTopics'), function (req, res) {
  ...
  api.comment.all({forum: req.params.forum}, function (err, comments) {
    if (err) {
      log('Error found: %j', err)
      return res.redirect('/500')
    }

    var topicsId = comments
      .map(function (comment) { return comment.reference })
      .filter(function (id, i, ids) { return ids.indexOf(id) === i })
    Topic.find(
      {'_id': {$in: topicsId}},
      function (err, topics) {
        var topicsObj = {}
        topics.forEach(function (topic) { topicsObj[topic.id] = topic })
        var csvArray = comments.map(function (comment) {
          var topic = topicsObj[comment.reference]
          var replies = (comment.replies.length > 0)
            ? comment.replies
            : [{text: '', author: {fullName: '', email: ''}}]
...
app.use('/comment', accepts('application/json'))
...
app.post('/comment/:id/unvote', restrict, privileges('canVoteAndComment'), function (req, res) {
  log('Request /comment/%s/unvote', req.params.id)
...
function getForum (req, res, next) {
  api.forum.findById(req.params.forum, function (err, forum) {
    if (err || !forum) {
      log('Error found: %j', err)
      return res.redirect('/500')
    }
