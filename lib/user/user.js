...
user.canCreateForum = function () {
  if (!user.loaded()) throw new Error('User must be loaded first.')

  if (config.multiForum) {
    if (config.restrictForumCreation && !user.staff) return false
  } else {
    if (config.defaultForum || !user.staff) return false
  }
