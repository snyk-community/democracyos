...
page('/forums/new', user.required, canCreateForum, () => {
...
function canCreateForum (ctx, next) {
  if (config.multiForum) {
    if (config.restrictForumCreation && !user.staff) {
      log('Forums can be created by a staff members only.')
      return page.redirect('/404')
    }
  } else {
    if (config.defaultForum || !user.staff) {
      log('Forum can be created by a staff members only.')
      return page.redirect('/404')
    }
  }
