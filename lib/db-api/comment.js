...
exports.all = function all (options, fn) {
  ...
  if (options) {
    if (options.forum) {
      Topic.all({forum: options.forum}, function (err, topics) {
        if (err) {
          log('Found error %j', err)
          return fn(err)
        }
        var topicsId = topics.map(function (topic) {
          return topic.id
        })
        Comment
          .find({'reference': {$in: topicsId}})
          ...
          .exec(function (err, comments) {
            ...
            return fn(null, comments)
          })
      })
    } else {
      log('Unknown option in comment.all method')
      return fn(new Error('Unknown option in comment.all method'))
    }
  }
...
exports.getFor = function getFor (query, paging, fn) {
  ...
  var pagination = {
    page: +paging.page || 0,
    limit: +paging.limit || config.commentsPerPage || 100,
    sort: paging.sort || '-score'
  }

  Comment
    .find(query)
    .sort(pagination.sort || 'score')
