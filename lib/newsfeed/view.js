...
export default class Newsfeed extends view('appendable', 'withEvents') {
  constructor (options = {}) {
    ...
    if (!config.restrictForumCreation) this.loadForumInfo()
  }
  ...
  loadForumInfo () {
    ...
    let render = (forums) => {
      let info = domRender(forumInfoTemplate, { forums: forums })
    ...
    forumStore.findAll({'privileges.canChangeTopics': true})
      .then(render)
      .catch((err) => {
        if (err.status === 404) render()
        else if (err.status === 403) render()
