...
const valid = (function () {
  const pages = ['profile', 'password', 'notifications', 'forums']
  return function (ctx, next) {
    var page = ctx.params.page || 'profile'
    ctx.valid = ~pages.indexOf(page)
    if (page === 'forums' && !user.canCreateForum()) ctx.valid = false
    return next()
  }
})()
...
page('/settings/:page?', user.required, valid, external, (ctx, next) => {
